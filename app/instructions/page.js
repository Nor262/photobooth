...
function InstructionsContent() {
  ...
  useEffect(() => {
    ...
    if (shotsFromUrl) {
      ...
      setShots(parseInt(shotsFromStorage));
    } else {
  ...
  const handleContinue = () => {
    if (instructionsRead) {
      ...
      setTimeout(() => {
        router.push(`/photoCapture?shots=${shots}`);
      }, 800);
    } else {
      alert("Please confirm that you've read the instructions before continuing!");
    }
  };
  ...
  return (
    <main className="h-screen flex items-center justify-center relative overflow-hidden">
      ...
      {/* Main content */}
      <motion.div
        className="z-10 w-full max-w-sm bg-white rounded-lg shadow-md p-4 border border-blue-100"
        initial="hidden"
        animate={exitAnimation ? 'exit' : 'visible'}
        variants={fadeIn}
      >
        <motion.div variants={fadeIn} className="mb-3 text-center">
          <Camera size={20} className="text-blue-500 mx-auto mb-1" />
          <h1 className="text-xl font-medium text-gray-800">Getting Ready</h1>
          <p className="text-gray-500 text-xs">
            {shots && `You've selected ${shots} photo${shots > 1 ? 's' : ''}`}
          </p>
        </motion.div>

        <motion.div variants={fadeIn} className="mb-4 space-y-2 bg-gray-50 p-3 rounded-md border border-blue-50">
          <h2 className="font-medium text-gray-700 text-sm">Instructions:</h2>

          <div className="space-y-2 text-xs">
            <div className="flex items-start">
              <div className="text-blue-400 mr-2 flex-shrink-0">1.</div>
              <p className="text-gray-700">Each photo has a <span className="font-medium text-blue-600">3-second timer</span> to get ready.</p>
            </div>

            <div className="flex items-start">
              <div className="text-blue-400 mr-2 flex-shrink-0">2.</div>
              <p className="text-gray-700">A countdown will appear before each shot is taken.</p>
            </div>

            <div className="flex items-start">
              <div className="text-blue-400 mr-2 flex-shrink-0">3.</div>
              <p className="text-gray-700">You'll see a preview after each photo is captured.</p>
            </div>

            <div className="flex items-start">
              <div className="text-blue-400 mr-2 flex-shrink-0">4.</div>
              <p className="text-gray-700">Please stay within the frame indicated on screen.</p>
            </div>

            <div className="flex items-start">
              <div className="text-blue-400 mr-2 flex-shrink-0">5.</div>
              <p className="text-gray-700">After all photos are taken, you'll have the option to download them.</p>
            </div>
          </div>

          <div className="pt-2 mt-1 border-t border-gray-200">
            <div className="font-medium text-gray-700 mb-1 text-xs">Timer per photo:</div>
            <div className="flex items-center justify-center bg-white rounded-md p-1 border border-blue-100">
              <div className="text-lg font-medium text-blue-600">3</div>
              <div className="text-gray-500 ml-1 text-xs">seconds</div>
            </div>
          </div>
        </motion.div>
        ...
        <motion.div
          ...
          whileTap={{ scale: 0.98 }}
        >
          <div className={`w-4 h-4 rounded border mr-2 flex items-center justify-center transition-colors ${instructionsRead ? 'bg-blue-500 border-blue-500' : 'border-gray-300'
            }`}>
            {instructionsRead && <CheckCircle size={12} className="text-white" />}
          </div>
          <div className="text-gray-700 text-xs">I have read and understood the instructions</div>
        </motion.div>

        <motion.button
          onClick={handleContinue}
          disabled={!instructionsRead}
          className={`w-full py-2 rounded-md text-xs font-medium transition-all ${instructionsRead
            ? 'bg-blue-500 text-white hover:bg-blue-600'
            : 'bg-gray-200 text-gray-400 cursor-not-allowed'
            }`}
          variants={fadeIn}
          whileHover={instructionsRead ? { scale: 1.01 } : {}}
          whileTap={instructionsRead ? { scale: 0.99 } : {}}
        >
          Start Photo Session
          {instructionsRead && (
      ...
      <motion.div
        className="text-xs text-blue-500 text-center absolute bottom-4 z-10"
        initial={{ opacity: 0 }}
        animate={{ opacity: 0.8 }}
        transition={{ delay: 0.8 }}
      >
        You can adjust your position during the countdown
      </motion.div>
...
export default function BoothInstructions() {
  return (
    <Suspense fallback={
      <div className="h-screen flex items-center justify-center bg-gradient-to-b from-blue-50 to-white">
        <div className="text-blue-500 animate-pulse text-center">
